package dag.hpt
{
    ...
    {	
		...
		public function checkSku():void{
			...
			request = new URLRequest("http://api.remix.bestbuy.com/v1/products/" + sku + ".xml?apiKey=bnt2fgfhs9q53exuqhpfqwy3&show=name,salePrice,modelNumber,largeFrontImage");
		...
		private function productLoaderCompleteHandler(event:Event):void {
			...
			productName = productXml.name.text();
			model = productXml.modelNumber.text();
			trace(productXml.modelNumber.text());
			trace(model);
			productImage = productXml.largeFrontImage.text();
			dispatchEvent(customEvent);
